...
export async function appendTuiPrompt(port: string, payload: TUIAppendPrompt): Promise<void> {
...
export async function showTuiToast(port: string, payload: TUIShowToast): Promise<void> {
  await fetch(`http://localhost:${port}/tui/show-toast`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  })
